...
Look for the Build number. (Note: If you have a Samsung device, you must first tap Software information to find the Build number).
Tap the Build number exactly 7 times in quick succession. 
A toast message will appear saying "You are now a developer!"Enter your phone's lock screen PIN, password, or pattern when prompted to confirm.

After that

Go back to the main Settings menu.
Navigate to System (On some phones like Xiaomi/POCO, this might be under Additional settings).
Tap Developer options.Scroll down to the Debugging section and toggle the switch for USB debugging to the ON position.
When the "Allow USB debugging?" prompt appears, tap OK or Allow
